...
Uses
    ...
    uPacketListeners, uMaintenanceTasks, uEncryption, EventLog, Generics.Collections, Dialogs, System.StrUtils, System.Types;
...
Type
    ...
    protected
        ...
        procedure CreateDeviceData(var Device: TDeviceCacheEntry; const IPAddress: String; Port: Word; SequenceNumber: Byte; EncryptionType: Byte; Socket: TUDPChildSocket);
    ...
    protected
        ...
        procedure SetDevicePropertiesOnMemory(const Device: TDeviceCacheEntry; StringModelFirmware: string);
...
procedure TDataProcessingThread.SetDevicePropertiesOnMemory(const Device: TDeviceCacheEntry; StringModelFirmware: string);
...
begin
  ...
  begin
    ...
    // Divide la última parte utilizando la coma como delimitador

    modelAndFirmware := SplitString(parts[High(parts)], ',');

    // Encuentra la posición del guión en el modelo

    dashPos := Pos('-', modelAndFirmware[0]);

    // Asignar los datos extraídos a las propiedades del dispositivo

    if dashPos > 0 then

      UpdatedDevice.Model := Copy(modelAndFirmware[0], dashPos + 1, MaxInt) // Copia desde la posición del guión hasta el final
...
function TDataProcessingThread.BytesToString(const Bytes: array of Byte): string;
...
begin
    ...
    if Device.Secured = false then
      ...
      Exit; // Salir de la función si Device.PrivateKey es nulo
...
begin
    ...
    // PORQUE IP ADDRESSS VENDRÍA VACIO?? DETECTANMOS ALGUN CASO? SI NO BORRAR

      // Agrega una validación para asegurarte de que IPAddress no esté vacío

    if IPAddress = '' then begin

        // Puedes manejar el error aquí, por ejemplo, registrando un mensaje de error o lanzando una excepción

        LogActivity('', Packet.Header.SourceMAC, 99, 'IPAddress is empty for DEVICE.');

        EventLogReportEvent(elWarning, Format('IPAddress is empty for DEVICE. MAC Address: %s', [MACToStr(Packet.Header.SourceMAC)]));
    ...
    // SOLO DEBERÍA HACER ESTE CHEQUEO ANTE KEEP ALIVE, SERVER TIME Y GET PRIVATE KEY? TODO ESTE CODIGO SE EJECUTA

    // INDEPENDIENTEMENTE DE SI EL MENSAJE ES VALIDO, ESTÁ CORRECTAMENTE ENCRIPTADO O SI HAY QUE RESPONDERLO
    ...
    begin
        CreateDeviceData(Device, IPAddress, Port, Packet.Header.SequenceNumber, Device.EncryptionType, Socket);
        LogActivity('', Packet.Header.SourceMAC, 99, 'Device reported (not linked): ' + Device.RemoteIP);
    end;
    ...
    case Packet.Header.CommandID of
        ...
        HA_KEEP_ALIVE:
            ...
            begin
                ...
                try
                  ...
                  StringModelFirmware := BytesToString(Packet.Join.JoinRequest.SSID); // Capturo modelo y firmware del keep_alive.

                  SetDevicePropertiesOnMemory(Device, StringModelFirmware);

                  if Device.Linked then
                  begin
                  HomeCloudPrivateService.UpdateLastDeviceActivity(Device.DeviceID, MACToStr(Device.MAC)); // Notifico estado online del dispositivo al BE.
                  end;
                except
                  ...
                  begin
                    ...
                    LogActivity('EXCEPTION', Device.MAC, 99, 'Ex Calss: ' + E.ClassName + ' || Exception message: ' + E.Message);
        ...
        HA_JOIN:
            ...
            ProcessJoin(EncryptionType, Packet, Device, Socket); //TODO: AGREGAR DISPARO DIRECTO PARA SETEAR LA CLAVE.
        ...
        HA_EP_MANAGER_COMMAND:
           try
           LogActivity('HA_EP_MANAGER_COMMAND', Device.MAC, 99, IntToStr(Device.DeviceID) + ' ' + IntToStr(Packet.Header.SourceEndpoint) + ' ' + IntToStr(Packet.EndpointManager.EnrollmentResult.EnrollmentStatus) + ' ' + IntToStr(Packet.EndpointManager.EnrollmentResult.ManagedEndpointNumber) + ' ' + IntToStr(Packet.EndpointManager.EnrollmentResult.Descriptor.EndpointType) + ' ' + TEncoding.Default.GetString(Packet.EndpointManager.EnrollmentResult.Descriptor.Name) + ' ' + TEncoding.Default.GetString(Packet.EndpointManager.EnrollmentResult.Descriptor.Model) + ' ' + TEncoding.Default.GetString(Packet.EndpointManager.EnrollmentResult.Descriptor.ID));
        ...
        HA_DOORLOCK_COMMAND:
            try
           LogActivity('HA_DOORLOCK_COMMAND', Device.MAC, 99, IntToStr(Device.DeviceID) + ' ' + IntToStr(Packet.Header.SourceEndpoint) + ' ' + IntToStr(Packet.EndpointManager.EnrollmentResult.EnrollmentStatus) + ' ' + IntToStr(Packet.EndpointManager.EnrollmentResult.ManagedEndpointNumber) + ' ' + IntToStr(Packet.EndpointManager.EnrollmentResult.Descriptor.EndpointType) + ' ' + TEncoding.Default.GetString(Packet.EndpointManager.EnrollmentResult.Descriptor.Name) + ' ' + TEncoding.Default.GetString(Packet.EndpointManager.EnrollmentResult.Descriptor.Model) + ' ' + TEncoding.Default.GetString(Packet.EndpointManager.EnrollmentResult.Descriptor.ID));
...
procedure TDataProcessingThread.CreateDeviceData(var Device: TDeviceCacheEntry; const IPAddress: String; Port: Word; SequenceNumber: Byte; EncryptionType: Byte; Socket: TUDPChildSocket);
...
procedure TDataProcessingThread.UpdateEndpointValues(var E: TEndpointCacheEntry; const Values: ArrayOfEndpointValue);
